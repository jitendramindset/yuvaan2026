...
import { LayoutDashboard, Edit3, Plus, X } from "lucide-react";
...
export default function DashboardPage() {
  ...
  // Quick-add widget directly from dashboard
  ...
  return (
    <OSShell dashboardContent={
      <div className="h-full overflow-y-auto space-y-4 pb-8 px-0">
      {/* OS Status bar */}
      <div
      ...
      >
        ...
        <div>
          ...
          {today && <span className="text-xs ml-2" style={{ color: "var(--muted)" }}>{today}</span>}
        </div>
      ...
      {showAdd && (
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold">Add a widget to dashboard</span>
            <button onClick={() => setShowAdd(false)} className="text-xs" style={{ color: "var(--muted)" }}>
              <X size={14} />
            </button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {WIDGET_CATALOGUE.map((entry) => (
              ...
              >
                <div className="text-xl mb-1">{entry.icon}</div>
                <div className="font-semibold">{entry.label}</div>
                <div style={{ color: "var(--muted)", fontSize: 10, lineHeight: 1.4 }}>{entry.desc}</div>
              </button>
            ))}
          </div>
          <p className="text-xs mt-3" style={{ color: "var(--muted)" }}>
            Widget is added at the bottom. Go to <Link href="/builder" className="underline" style={{ color: "var(--accent)" }}>Builder</Link> to reposition and resize.
          </p>
        </div>
      )}
